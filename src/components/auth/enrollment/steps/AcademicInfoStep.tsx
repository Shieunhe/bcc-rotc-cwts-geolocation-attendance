    ...
    <>
      <div>
        ...
        <select
          ...
          className={`${selectClass} ${!form.course ? "text-base font-bold text-black" : ""}`}
        >
          <option
            value=""
            disabled
            style={{ backgroundColor: "#d1d5db", color: "#000000", fontSize: "16px" }}
          >
            Select Course
          </option>
          ...
          {nstpPrograms.map((program) => {
            ...
            return (
              <button key={program} type="button"
                ...
                disabled={isDisabled}
                className={`py-2.5 rounded-2xl border-2 text-sm font-semibold transition-all
                  ${isDisabled
