...
export default function BattalionAttendanceBox({
  ...
  const filtered = students.filter((row) => {
    ...
    if (filterPlatoon && String(row.platoon) !== filterPlatoon) return false;
  ...
  for (const row of filtered) {
    const s = getStatus(row.student.uid, recordMap, graceOver);
      ...
      <div className="p-5 space-y-4">
        {students.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-sm text-gray-400 font-medium">No students assigned to this battalion.</p>
          </div>
        ) : (
          <>
            {/* Overview cards */}
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                <div className="relative">
                  <select value={selectedType} onChange={(e) => onTypeChange(e.target.value as "in" | "out")}
                    className={`appearance-none px-3 py-1.5 pr-7 rounded-lg border border-gray-200 bg-gray-50 text-[11px] font-semibold text-gray-600 focus:outline-none focus:ring-2 ${accentRing} transition`}>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-9 pr-3 py-2 rounded-xl border border-gray-200 bg-gray-50 text-xs font-medium text-gray-700 placeholder:text-gray-400 focus:outline-none focus:ring-2 ${accentRing} transition`}
                />
              </div>
            </div>
                ...
                <>
                  ...
                  <div className="hidden sm:block divide-y divide-gray-100">
                    <div className="grid grid-cols-[1fr_auto_auto_auto] gap-2 px-4 py-2 bg-gray-50 border-b border-gray-100 text-[10px] font-bold text-gray-400 uppercase tracking-wide">
                      <span>Student</span>
                      <span className="w-20 text-center">Assignment</span>
                    ...
                    {sorted.map((row) => {
                      ...
                      return (
                        <div key={s.uid} className={`grid grid-cols-[1fr_auto_auto_auto] gap-2 items-center px-4 py-2.5 border-l-3 ${cfg.border}`}>
                          ...
                          <span className="text-[10px] text-gray-400 font-medium w-20 text-center truncate">
                            {row.company} · P{row.platoon}
                          </span>
                      ...
                      return (
                        <div key={s.uid} className={`px-4 py-3 border-l-3 ${cfg.border}`}>
                          ...
                          <div className="flex items-center gap-2 mt-1.5">
                            <span className="text-[10px] text-gray-400 font-medium">{row.company} · P{row.platoon}</span>
                            <span className="text-gray-300">·</span>
            ...
            <p className="text-[10px] text-gray-400 text-right">{sorted.length} of {students.length} students shown</p>
          </>
        )}
      </div>
    </div>
